...
function superpixels = computeSuperpixels( options,frames )
    ...
    if( exist( filename, 'file' ) )
        ...
        for(index = 1: nframes)
            ...
            %options.regnum =500;options.m = 20;
            [ Label, Sup1, Sup2, Sup3, k ] = SLIC( double(frameVecR),...
                double(frameVecG), double(frameVecB), frameAttr );                                             %#ok<NASGU>
        ...
        %把变量superpixels保存到FileName指定的文件中去。
    end
