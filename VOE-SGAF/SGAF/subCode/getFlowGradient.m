% Function to compute the gradient of the given optical flow计算给定光流的梯度
%
...
function result = getFlowGradient( flow )
    ...
    if( iscell( flow ) )%flow <240x320x2 double>
        framesNumber = length( flow );%320
        ...
        grad = zeros( height, width, 2, 'single' );%2个
        if( ~isfloat( flow ) )
            flow = single( flow );
        end
